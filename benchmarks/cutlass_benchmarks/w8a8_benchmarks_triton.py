...
# bench
def bench_int8(dtype: torch.dtype, MKNs: Iterable[Tuple[int, int, int]], tp_size: Optional[int] = None) -> List[triton.testing.Benchmark]:
    assert dtype == torch.int8
    configs = []
    for with_bias in [True, False]:
        for with_azp_per_tensor in [True, False]:
            for with_azp_per_token in [True, False]:
                plot_name = f"int8_gemm-{dtype}-bias-{with_bias}-azp-per-tensor-{with_azp_per_tensor}-azp-per-token-{with_azp_per_token}"
    ...
    def _bench(dtype: torch.dtype, m: int, k: int, n: int, with_bias: bool, with_azp_per_tensor: bool, with_azp_per_token: bool, version: str):
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(bench_fn, quantiles=[0.5, 0.2, 0.8])
        return ms, max_ms, min_ms

    triton.testing.perf_report(configs)(_bench).run()

# TODO support fp8
def bench(dtype: torch.dtype, MKNs: Iterable[Tuple[int, int, int]]) -> List[triton.testing.Benchmark]:
    if dtype == torch.int8:
...
def make_output(data, MKNs: Iterable[Tuple[int, int, int]], base_description: str, timestamp=None):
